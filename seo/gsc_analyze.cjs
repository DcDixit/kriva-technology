...
const FILES = {
 queries: "queries.csv", pages: "pages.csv", countries: "countries.csv", queriesByCountry: "queries-by-country.csv", opportunities: "keyword-opportunities.csv", };
...
function main() {
 ...
 const report = {
 generatedAt: new Date().toISOString(), dataFiles: Object.fromEntries(
 Object.entries(FILES).map(([k, v]) => [k, fs.existsSync(path.join(DATA_DIR, v))])
 ), usTrucking: { mappedQueries: [], gaps: [], topPages: [] }, marketComparison: {}, pagePerformance: [], urlValidationCandidates: [], recommendations: [], };
 ...
 query, impressions, clicks, position, suggestedPages: cluster.pages.map((p) => p.path), });
 ...
 query, impressions, position, passesRuleA: impressions > 0, note: "Rule A: impressions exist, still requires editorial review before any new URL", });
 }
 }
 ...
 url, clicks: num(row.Clicks ?? row.clicks), impressions: num(row.Impressions ?? row.impressions), position: num(row.Position ?? row.position), });
 }
 ...
 label, marketPage: PAGE_MAP.markets[code]?.path, clicks: num(row.Clicks ?? row.clicks), impressions: num(row.Impressions ?? row.impressions), position: num(row.Position ?? row.position), ctr: row.CTR ?? row.ctr ?? "", };
 }
 }
 }
 ...
 .map((r) => ({
 query: r[qk], impressions: num(r.Impressions ?? r.impressions), position: num(r.Position ?? r.position), cluster: matchCluster(r[qk])?.id ?? null, }))
 ...
 report.urlValidationCandidates.push({
 query: row[qk], position, cpc, passesRuleB: true, note: "Rule B: positions 11–20, CPC > $8, review before any new URL", });
 }
 }
 }
