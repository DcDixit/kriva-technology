...
const QA = [
  [
    ...
    'No. KRIVA is remote-first from Ahmedabad, India, and we do not claim a local presence we do not have. Gulf Standard Time is two and a half hours ahead of us, so a normal UAE working day overlaps almost entirely with ours — in practice that means same-day replies and live sessions at ordinary hours for both sides.',
  ],
  [
    'Can you build Arabic and English in the same product?',
    'Yes, and it is much cheaper to plan for at the start. We structure layouts, components, and content for bidirectional text from the wireframe stage. Translation itself is usually handled by your team or a translation partner; what we own is making sure the interface does not break when the direction flips.',
  ],
  [
    'Which accounting systems can you integrate with?',
    'QuickBooks and Xero are the two we work with most, using their official APIs. For regional ERPs and bank feeds we work against whatever documented API exists, and we say plainly when a system has no viable integration path rather than building a fragile scraper.',
  ],
  [
    'Who owns the code and the designs?',
    'You do. Repositories, Figma files, and documentation transfer to you at close or earlier if you prefer. There is no licensing arrangement and no lock-in to us for future work.',
  ],
  [
    'Can you improve an existing system instead of rebuilding it?',
    'Usually, and it is normally the better option. We assess what your current platform can actually support, then build beside it in phases behind feature flags so the desk keeps running while the work lands. A full rebuild only makes sense when the platform genuinely cannot carry the workload.',
  ],
];
...
const node = {
  '@type': 'FAQPage',
  '@id': 'https://krivatechnologies.com/markets/uae#faq',
