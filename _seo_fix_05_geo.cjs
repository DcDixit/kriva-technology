...
const MARKETS = {
 'kriva-market-us.html': { path: '/markets/us', region: 'US', place: 'United States', locale: 'en_US' }, 'kriva-market-uk.html': { path: '/markets/uk', region: 'GB', place: 'United Kingdom', locale: 'en_GB' }, 'kriva-market-uae.html': { path: '/markets/uae', region: 'AE', place: 'United Arab Emirates', locale: 'en_AE' }, 'kriva-market-ca.html': { path: '/markets/ca', region: 'CA', place: 'Canada', locale: 'en_CA' }, 'kriva-markets-index.html': { path: '/markets', region: null, place: null, locale: 'en_US' }, };

// identical block on every cluster member, x-default on the locale-neutral hub
const CLUSTER = [
 `<link rel="alternate" hreflang="en-US" href="${ORIGIN}/markets/us">`, `<link rel="alternate" hreflang="en-GB" href="${ORIGIN}/markets/uk">`, `<link rel="alternate" hreflang="en-AE" href="${ORIGIN}/markets/uae">`, `<link rel="alternate" hreflang="en-CA" href="${ORIGIN}/markets/ca">`, `<link rel="alternate" hreflang="x-default" href="${ORIGIN}/markets">`, ].join('\n');
...
for (const [file, cfg] of Object.entries(MARKETS)) {
 ...
 h = h.replace(
 /(<meta name="geo\.region"[^>]*>)/i, `${CLUSTER}\n$1`
 );

 if (cfg.region) {
 h = h.replace(/<meta name="geo\.region"[^>]*>/i, `<meta name="geo.region" content="${cfg.region}">`);
 h = h.replace(
 /<meta name="geo\.placename"[^>]*>/i, `<meta name="geo.placename" content="${cfg.place}">`
 );
