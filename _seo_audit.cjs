...
const SCHEMA_TYPES = new Set([
 'Organization', 'ProfessionalService', 'LocalBusiness', 'Corporation', 'WebSite', 'WebPage', 'AboutPage', 'ContactPage', 'CollectionPage', 'FAQPage', 'QAPage', 'BreadcrumbList', 'ListItem', 'ItemList', 'Article', 'BlogPosting', 'NewsArticle', 'TechArticle', 'Blog', 'CreativeWork', 'Service', 'Offer', 'OfferCatalog', 'AggregateOffer', 'Product', 'Question', 'Answer', 'Person', 'ImageObject', 'VideoObject', 'PostalAddress', 'ContactPoint', 'Country', 'Place', 'GeoCoordinates', 'SpeakableSpecification', 'SearchAction', 'EntryPoint', 'Review', 'Rating', 'AggregateRating', 'HowTo', 'HowToStep', 'JobPosting', 'SiteNavigationElement', 'Brand', 'DefinedTerm', 'DefinedTermSet', 'Occupation', 'EmployeeRole', 'MonetaryAmount', 'Audience', ]);
...
// ---------- helpers ----------
const rx = {
 head: /<head[\s\S]*?<\/head>/i, title: /<title[^>]*>([\s\S]*?)<\/title>/i, canonical: /<link[^>]+rel=["']canonical["'][^>]*>/gi, href: /href=["']([^"']+)["']/i, // quote-aware: the value may itself contain the *other* quote char (e.g. apostrophes)
...
for (const file of htmlFiles) {
 ...
 const p = {
 file, url, bytes: html.length, title: decode((html.match(rx.title) || [, ''])[1]), desc: decode(metaAll(head, 'description')[0] || ''), descCount: metaAll(head, 'description').length, robots: metaAll(head, 'robots')[0] || '', ogTitle: metaAll(head, 'og:title'), ogDesc: metaAll(head, 'og:description'), ogImage: metaAll(head, 'og:image'), ogUrl: metaAll(head, 'og:url')[0] || '', twCard: metaAll(head, 'twitter:card'), twImage: metaAll(head, 'twitter:image'), canonicals: (head.match(rx.canonical) || []).map((t) => (t.match(rx.href) || [, ''])[1]), hreflang: [...head.matchAll(/<link[^>]+hreflang=["']([^"']+)["'][^>]*href=["']([^"']+)["']/gi)].map(
 (m) => ({ lang: m[1], href: m[2] })
 ), head, body, html, };
 ...
 lvl: +m[1], text: decode(m[2].replace(/<[^>]+>/g, ' ')), }));
 ...
 return {
 href: m[1], text: decode(m[2].replace(/<[^>]+>/g, ' ')) || decode(alts), tag: m[0], };
 });
...
// hreflang reciprocity: every member of a cluster must advertise the identical set, // or Google discards the whole cluster
{
 const sig = (p) =>
 p.hreflang
