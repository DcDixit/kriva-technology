...
Scope: all 57 indexable pages, `robots.txt`, `sitemap.xml`, `feed.xml`, `llms.txt`, `vercel.json` routing/headers, static asset payload, and GA4 instrumentation.
...
chains, broken internal links (0), orphan pages (0), duplicate titles/descriptions/H1s (0), missing `alt` (0), missing `width`/`height` (0), GA4 coverage (58/58 pages, single
...
**Important caveat:** this audit is code-side only. I had no access to Search Console or GA4 data, so I could not confirm *which* queries, pages, or countries actually declined, whether there was a
