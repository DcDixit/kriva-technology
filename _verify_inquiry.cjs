...
const BRIEF = {
 inquiry_type: "project_brief", name: "KRIVA QA Test", email: "qa.test@example.com", company: "Krishuweb", site: "https://krivatechnologies.com", ptype: "Trucking & logistics (US)", market: "United States", service: "Mobile App Design", budget: "$5k - $15k", timeline: "ASAP (within 4 weeks)", details: "Automated test inquiry, please ignore. Verifying Project Brief delivery.", };

const FIT = {
 inquiry_type: "fit_call", name: "KRIVA QA Test", email: "qa.test@example.com", company: "Krishuweb", ptype: "SaaS product (UK / US)", details: "Automated fit-call test, please ignore.", timeline: "ASAP (within 4 weeks)", };

const PAGE = {
 inquiry_type: "page_inquiry", name: "KRIVA QA Test", email: "qa.test@example.com", company: "Krishuweb", page: "/", ptype: "Website inquiry", details: "Automated page-inquiry test, please ignore.", };
...
function postJson(path, payload) {
 ...
 "Content-Type": "application/json", Accept: "application/json", "Content-Length": Buffer.byteLength(body), }, }, (res) => {
 let data = "";
...
function assertNoSecrets(raw) {
 ...
 const forbidden = [
 process.env.GMAIL_APP_PASSWORD, process.env.RESEND_API_KEY, process.env.WEB3FORMS_ACCESS_KEY, "formsubmit.co", ].filter(Boolean);
...
async function main() {
 ...
 inquiry_type: "page_inquiry", name: "Bot", email: "bot@example.com", website_hp: "spam", });
 ...
 ["project_brief", BRIEF], ["fit_call", FIT], ["page_inquiry", PAGE], ]) {
 const api = await postJson("/api/inquiry", payload);
 assertNoSecrets(api.raw);
 const hasRelay = !!(api.json && api.json.relay);
 results.push({
 label, apiStatus: api.status, apiOk: api.json.ok, channel: api.json.channel || null, hasRelay, error: api.json.error || null, });
