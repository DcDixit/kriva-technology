...
PAGE_CURRENT = {
 ...
 "kriva-solution-trucking.html": "trucking", "kriva-solution-saas.html": "saas", "kriva-solution-accounting.html": "integration", "kriva-solution-car-transport.html": "trucking", "kriva-services-index.html": "services", "kriva-work-index.html": "work", "kriva-case-fleetflow.html": "work", "kriva-contact.html": None, }
...
NAV_CSS_START_PATTERNS = [
 re.compile(r"/\* ═══ NAV[^*]*\*+/.*?@media\(min-width:901px\)\{\.sheet\{display:none\}\}", re.S), re.compile(r"/\* ══ shared nav / sheet ══ \*/.*?@media\(min-width:901px\)\{\.sheet\{display:none\}\}", re.S), re.compile(r"/\*\s*══\s*shared nav.*?\*/.*?@media\(min-width:901px\)\{\.sheet\{display:none\}\}", re.S), ]

# Broader: from .nav{position:fixed through mobile sheet media queries before next major section
NAV_BLOCK = re.compile(
 r"/\*[^*]*nav[^*]*\*/\s*\.nav\{position:fixed.*?(?=\n/\*|\n\.hero|\n\.ph|\n\.crumbs|\n@media \(prefers-reduced-motion)", re.S | re.I, )

FOOTER_CSS = re.compile(
 r"/\*\s*═══════════ FOOTER ═══════════\s*\*/.*?@media\(max-width:820px\)\{\.fgrid\{grid-template-columns:1fr 1fr\}\}", re.S, )

NOTICE = re.compile(r"(?:<!--[^>]*[Dd]emo[^>]*-->\s*)?<div class=\"notice\">.*?</div>\s*", re.S)

HEADER_SHEET = re.compile(
 r"<header class=\"nav\" id=\"nav\">.*?</header>\s*<div class=\"sheet\" id=\"sheet\"[^>]*>.*?</div>\s*(?=<main)", re.S, )

FOOTER = re.compile(r"<footer>.*?</footer>", re.S)

# Remove old burger/sheet handlers that conflict with chrome.js
OLD_NAV_JS = re.compile(
 r"/\*\s*───────── nav:.*?(?=/\*\s*───────── HERO|/\*\s*nav\s*\*/|const con |/\*\s*cases|/\*\s*accordion|/\*\s*form|/\*\s*filter|/\*\s*console|/\*\s*machine|/\*\s*chapters|/\*\s*scroll)", re.S, )
OLD_NAV_JS2 = re.compile(
 r"/\*\s*nav\s*\*/\s*const nav = document\.getElementById\('nav'\);.*?sheet\.addEventListener\('click'.*?\);\s*addEventListener\('keydown'.*?\);\s*", re.S, )
OLD_NAV_JS3 = re.compile(
 r"const nav = document\.getElementById\('nav'\);\s*let lastY = 0;.*?addEventListener\('keydown', e=>\{ if\(e\.key === 'Escape'.*?\}\);\s*", re.S, )
...
def strip_old_nav_css(css_and_html: str) -> str:
 ...
 text = re.sub(
 r"/\* build-note strip:.*?\*/\s*\.notice\{.*?\}\s*\.notice \.wrap\{.*?\}\s*", "", text, count=1, flags=re.S, )
...
def strip_old_nav_js(html: str) -> str:
 ...
 text = re.sub(
 r"const links = \[\.\.\.document\.querySelectorAll\('#navLinks a'\)\];.*?targets\.forEach\(t=>secIO\.observe\(t\)\);\s*", "", text, count=1, flags=re.S, )
 return text
